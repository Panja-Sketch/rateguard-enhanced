...
#      (untagged) production rating-engine URL. It also sets both
#      RATEGUARD_*_CONNECTOR_AUDIENCE variables to that same stable URL: the
#      Google ID-token audience is an explicit setting (a tagged candidate
#      endpoint is rejected by Cloud Run as an audience), and production
#      startup fails closed if it is missing or inconsistent with the endpoint.
...
gcloud run services update rateguard-worker --region "$REGION" --project "$PROJECT_ID" \
  --update-env-vars "RATEGUARD_FIRESTORE_COLLECTION=${PROD_FIRESTORE_COLLECTION},RATEGUARD_GCS_BUCKET=${PROD_GCS_BUCKET},RATEGUARD_BIGQUERY_DATASET=${PROD_BIGQUERY_DATASET},RATEGUARD_BIGQUERY_PORTFOLIO_TABLE=${PROD_BIGQUERY_PORTFOLIO_TABLE},RATEGUARD_BIGQUERY_RESULTS_TABLE=${PROD_BIGQUERY_RESULTS_TABLE},RATEGUARD_PUBSUB_TOPIC=${PROD_TOPIC},RATEGUARD_PUBSUB_SUBSCRIPTION=${PROD_SUBSCRIPTION},RATEGUARD_RATING_ENGINE_CONNECTOR_BASE_URL=${PROD_RATING_ENGINE_URL},RATEGUARD_VENDOR_GATEWAY_CONNECTOR_BASE_URL=${PROD_RATING_ENGINE_URL},RATEGUARD_RATING_ENGINE_CONNECTOR_AUDIENCE=${PROD_RATING_ENGINE_URL},RATEGUARD_VENDOR_GATEWAY_CONNECTOR_AUDIENCE=${PROD_RATING_ENGINE_URL}"
